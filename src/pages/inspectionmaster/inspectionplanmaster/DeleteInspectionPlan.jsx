...
const DeleteInspectionPlan = ({ open, onClose, plan, onDelete }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`${BASE_URL}/api/inspection-plans/${plan._id}`, {
        headers: {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to delete inspection plan');
      }
    } catch (err) {
      console.error('Error deleting inspection plan:', err);
      setError(err.response?.data?.message || 'Failed to delete inspection plan. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <DialogContent sx={{ p: 3 }}>
        <Typography sx={{ fontSize: '0.875rem', color: COLORS.text.primary, mb: 1 }}>
          Are you sure you want to delete this inspection plan?
        </Typography>
        ...
        <Box sx={{ 
          ...
          <Stack direction="row" spacing={1.5} alignItems="center">
            <PlanIcon sx={{ color: '#991B1B', fontSize: '1.25rem' }} />
            <Box>
...
export default DeleteInspectionPlan;