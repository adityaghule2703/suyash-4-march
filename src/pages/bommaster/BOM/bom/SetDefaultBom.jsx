...
const SetDefaultBom = ({ open, onClose, bomId, onSuccess }) => {
  ...
  const handleSetDefault = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}/api/boms/${bomId}/set-default`,
        {},
        {
          headers: {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to set BOM as default');
      }
    } catch (err) {
      console.error('Error setting BOM as default:', err);
      setError(err.response?.data?.message || 'Failed to set BOM as default. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const isAlreadyDefault = bomDetails?.is_default === true;
  const canSetDefault = bomDetails?.status === 'Approved' && !isAlreadyDefault;
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ p: 2.5, bgcolor: COLORS.background.white }}>
        {fetchingDetails ? (
          ...
          <Stack spacing={2.5}>
            ...
            >
              <Typography sx={{ fontWeight: 600, fontSize: '0.8rem' }}>
                BOM set as default successfully!
              </Typography>
            </Alert>

            <Paper sx={{ p: 2, bgcolor: COLORS.background.light, borderRadius: 1.5, border: `1px solid ${COLORS.border}` }}>
              <Typography sx={{ fontSize: '0.75rem', fontWeight: 600, color: '#F59E0B', mb: 1.5 }}>
                Default BOM Details
              </Typography>
              <Grid container spacing={1.5}>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  <Typography sx={{ fontSize: '0.7rem', color: COLORS.text.secondary }}>
                    <StarIcon sx={{ fontSize: '0.7rem', mr: 0.5, verticalAlign: 'middle' }} />
                    Is Default
                  </Typography>
                  <Chip
                    ...
                    label="Default BOM"
                    size="small"
                    sx={{ bgcolor: '#FEF3C7', color: '#D97706', fontSize: '0.7rem', mt: 0.5 }}
                  />
                </Grid>
              </Grid>
            </Paper>
          ...
          <Stack spacing={2.5}>
            ...
            >
              <Typography sx={{ fontWeight: 600, fontSize: '0.8rem' }}>
                This BOM is already set as default!
              </Typography>
              <Typography sx={{ fontSize: '0.7rem', mt: 0.5 }}>
                This BOM is currently the default version for this parent item.
              </Typography>
            </Alert>
          </Stack>
        ) : bomDetails && bomDetails.status !== 'Approved' ? (
          // Not Approved State
          <Stack spacing={2.5}>
            <Alert 
              severity="warning" 
            ...
            >
              ...
              <Typography sx={{ fontSize: '0.7rem', mt: 0.5 }}>
                Only approved BOMs can be set as default. Please approve this BOM first.
              </Typography>
            </Alert>
          </Stack>
        ) : bomDetails ? (
          ...
          <Stack spacing={2.5}>
            ...
            >
              <Typography sx={{ fontWeight: 600, fontSize: '0.8rem' }}>
                Are you sure you want to set this BOM as default?
              </Typography>
              <Typography sx={{ fontSize: '0.7rem', mt: 0.5 }}>
                This will make this version the default BOM for the parent item.
              </Typography>
            </Alert>
            ...
            {/* Warning about existing default */}
            <Alert 
              severity="info" 
              sx={{ borderRadius: 1.5, fontSize: '0.7rem' }}
              icon={<StarIcon />}
            >
              Setting this as default will replace any existing default BOM for this parent item.
            </Alert>
      ...
      <DialogActions sx={{
        ...
        {!success && canSetDefault && (
          <Button
            variant="contained"
            onClick={handleSetDefault}
            disabled={loading || fetchingDetails}
            startIcon={loading ? <CircularProgress size={16} /> : <StarIcon sx={{ fontSize: '1rem' }} />}
            sx={{
              height: 32,
              px: 2,
              borderRadius: 1.5,
