// src/pages/DeliveryChallan/components/Modals/EditPackingListDialog.js
...
const EditPackingListDialog = ({ open, onClose, deliveryChallan, onSuccess }) => {
  ...
  const fetchPackingList = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${BASE_URL}/api/packing-lists/dc/${deliveryChallan._id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      if (response.data.success && response.data.data) {
        ...
        setPackingListId(packingData._id);
        setFormData({ packed_by: packingData.packed_by || '' });
        
        const transformedPackages = packingData.packages.map(pkg => ({
          package_no: pkg.package_no,
          package_type: pkg.package_type,
          dimensions_l_mm: pkg.dimensions_l_mm,
          dimensions_w_mm: pkg.dimensions_w_mm,
          dimensions_h_mm: pkg.dimensions_h_mm,
        ...
        setError('No packing list found for this delivery challan');
      }
    } catch (err) {
      console.error('Error fetching packing list:', err);
      setError(err.response?.data?.message || 'Failed to fetch packing list');
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`${BASE_URL}/api/packing-lists/${packingListId}`, payload, {
        headers: {
          'Authorization': `Bearer ${token}`,
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to update packing list');
      }
    } catch (err) {
      console.error('Error updating packing list:', err);
      setError(err.response?.data?.message || 'Failed to update packing list. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <DialogActions sx={{
        ...
        <Button
          ...
          disabled={loading || fetching || !packages.length}
          sx={{
            height: 36,
            px: 3,
            borderRadius: 1.5,
        ...
        >
          {loading ? 'Updating...' : 'Update Packing List'}
        </Button>
      </DialogActions>
    </Dialog>
  );
