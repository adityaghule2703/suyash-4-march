...
//     endpoint: `${BASE_URL}/candidates?status=selected`,
//     lastAttempt: null
//   });
...
//       setError(`Error: ${err.message}. Please check if the API server is running at ${BASE_URL}`);
//     } finally {
//       setLoading(false);
//     }
//   };
...
//   const isActionEnabled = (action, status, applicationStatus) => {
//     const actionStatusMap = {
//       initiateOffer: ['Pending', 'Rejected', 'selected'],
...
//               API Endpoint: {apiDetails.endpoint}<br />
//               Last Attempt: {apiDetails.lastAttempt || 'Not attempted'}<br />
//               Please ensure:
//               <ul>
//                 <li>The API server is running at {BASE_URL}</li>
//                 <li>The endpoint /api/candidates?status=selected is accessible</li>
  ...
  const fetchSelectedCandidates = async () => {
    ...
    try {
      ...
      const response = await fetch(apiUrl, {
        ...
        credentials: 'omit' // Changed from 'include' to 'omit' to avoid CORS issues with wildcard
      });
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Alert severity="info" sx={{ mt: 2 }}>
            <Typography variant="body2">
              ...
              API Endpoint: {apiDetails.endpoint}<br />
              Last Attempt: {apiDetails.lastAttempt || 'Not attempted'}<br />
              <strong>Authentication Required:</strong> This endpoint requires a valid authentication token.
              {!getAuthToken() && (
