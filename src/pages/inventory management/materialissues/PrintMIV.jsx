...
const PrintMIV = ({ open, onClose, data }) => {
  ...
  const fetchPrintData = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/api/miv/${data._id}/print`,
        {
          headers: { 
      ...
      if (err.response) {
        ...
        if (err.response.status === 404) {
          ...
          setError("You don't have permission to print this MIV");
        } else {
          setError(errorMsg);
        }
      } else if (err.request) {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>MIV_${printData?.document?.number || data?.miv_number || 'Print'}</title>
          <meta charset="utf-8" />
          <style>
            * {
              margin: 0;
          ...
          <Box id="print-content">
            ...
            <Box sx={{ textAlign: 'center', mb: 3, pb: 2, borderBottom: `3px solid ${COLORS.primary}` }}>
              <Typography sx={{ fontSize: '1.8rem', fontWeight: 700, color: COLORS.primary, mb: 1 }}>
                {displayData.document?.title || "MATERIAL ISSUE VOUCHER"}
              </Typography>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} md={4}>
                  ...
                  <Typography sx={valueStyle}>{displayData.document?.status || '-'}</Typography>
                </Grid>
              </Grid>
            </Box>
            ...
            {/* Work Order & Customer Section */}
            <Box sx={{ mb: 3 }}>
              <Typography sx={sectionTitleStyle}>WORK ORDER & CUSTOMER</Typography>
              <Grid container spacing={2}>
                <Grid item xs={12} md={4}>
                ...
                <Grid item xs={12} md={4}>
                  ...
                  <Typography sx={valueStyle}>{displayData.customer?.name || '-'}</Typography>
                </Grid>
                <Grid item xs={12} md={4}>
                  <Typography sx={labelStyle}>SO NUMBER</Typography>
                  <Typography sx={valueStyle}>{displayData.customer?.so_number || '-'}</Typography>
                </Grid>
              </Grid>
            </Box>
            ...
            {/* Department Section */}
            <Box sx={{ mb: 3 }}>
              <Typography sx={sectionTitleStyle}>DEPARTMENT</Typography>
              <Typography sx={valueStyle}>{displayData.department || '-'}</Typography>
            </Box>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={4}>
                  ...
                  <Typography sx={valueStyle}>{displayData.personnel?.issued_by || '-'}</Typography>
                </Grid>
                <Grid item xs={12} md={4}>
                ...
                <Grid item xs={12} md={4}>
                  ...
                  <Typography sx={valueStyle}>{displayData.personnel?.authorised_by || '-'}</Typography>
                </Grid>
                <Grid item xs={12} md={4}>
            ...
            {/* Items Table */}
            <Box sx={{ mb: 3 }}>
              <Typography sx={sectionTitleStyle}>MATERIAL ITEMS</Typography>
                ...
                <Table size="small">
                  <TableHead>
                    <TableRow sx={{ bgcolor: COLORS.primary }}>
                      ...
                      <TableCell sx={{ fontSize: '0.7rem', fontWeight: 600, color: COLORS.text.light }} align="right">BOM Qty</TableCell>
                      <TableCell sx={{ fontSize: '0.7rem', fontWeight: 600, color: COLORS.text.light }} align="right">Issued Qty</TableCell>
                      ...
                      <TableCell sx={{ fontSize: '0.7rem', fontWeight: 600, color: COLORS.text.light }} align="right">Total Cost</TableCell>
                  ...
                  <TableBody>
                    {(displayData.items || []).map((item, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell sx={{ fontSize: '0.7rem' }} align="right">{item.bom_required_qty || 0}</TableCell>
                        ...
                        <TableCell sx={{ fontSize: '0.7rem' }} align="right">{formatCurrency(item.total_cost || 0)}</TableCell>
            ...
            <Box sx={{ textAlign: 'right', mb: 3 }}>
              <Stack spacing={1} alignItems="flex-end">
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', width: 250 }}>
                  <Typography sx={{ fontWeight: 600, fontSize: '0.75rem' }}>Total Issue Cost:</Typography>
                  <Typography sx={{ fontSize: '0.875rem', fontWeight: 700, color: COLORS.primary }}>
                    {formatCurrency(displayData.totals?.total_issue_cost)}
                  </Typography>
                </Box>
                {displayData.totals?.total_weight > 0 && (
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', width: 250 }}>
                    <Typography sx={{ fontWeight: 600, fontSize: '0.75rem' }}>Total Weight:</Typography>
                    <Typography sx={{ fontSize: '0.75rem', fontWeight: 600 }}>{displayData.totals?.total_weight} Kg</Typography>
                  </Box>
                )}
              </Stack>
            </Box>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 4, pt: 2 }}>
              <Box sx={{ textAlign: 'center', width: 200 }}>
                <Typography sx={{ fontSize: '0.7rem', color: COLORS.text.secondary }}>Issued By</Typography>
                <Typography sx={{ mt: 3, pt: 1, borderTop: `1px solid ${COLORS.text.secondary}`, fontSize: '0.7rem' }}>
                  {displayData.personnel?.issued_by || 'Signature'}
                </Typography>
              </Box>
              <Box sx={{ textAlign: 'center', width: 200 }}>
                <Typography sx={{ fontSize: '0.7rem', color: COLORS.text.secondary }}>Received By</Typography>
                <Typography sx={{ mt: 3, pt: 1, borderTop: `1px solid ${COLORS.text.secondary}`, fontSize: '0.7rem' }}>
              ...
              <Box sx={{ textAlign: 'center', width: 200 }}>
                ...
                <Typography sx={{ mt: 3, pt: 1, borderTop: `1px solid ${COLORS.text.secondary}`, fontSize: '0.7rem' }}>
                  {displayData.personnel?.authorised_by || 'Signature'}
                </Typography>
              </Box>
            </Box>
