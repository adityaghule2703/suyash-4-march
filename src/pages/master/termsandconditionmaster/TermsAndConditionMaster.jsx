...
const ActionMenu = ({ item, onView, onEdit, onDelete, anchorEl, onClose, onOpen, permissions }) => {
  const canView = hasPermission(permissions, MODULES.TERMS_CONDITIONS_MASTER, PAGES.TERMS_AND_CONDITIONS, ACTIONS.VIEW);
  const canUpdate = hasPermission(permissions, MODULES.TERMS_CONDITIONS_MASTER, PAGES.TERMS_AND_CONDITIONS, ACTIONS.UPDATE);
  const canDelete = hasPermission(permissions, MODULES.TERMS_CONDITIONS_MASTER, PAGES.TERMS_AND_CONDITIONS, ACTIONS.DELETE);
...
const TermsAndConditionMaster = () => {
  ...
  const checkPermission = (action) => {
    ...
    return hasPermission(
      userPermissions,
      MODULES.TERMS_CONDITIONS_MASTER,
  ...
  const fetchTerms = useCallback(async () => {
    ...
    try {
      ...
      showNotification('Failed to load terms. Please try again.', 'error');
    } finally {
      setLoading(false);
    }
  ...
  const openDeleteTermDialog = (item) => {
    if (!canDelete) return;
    setSelectedItem(item);
      ...
      <Paper sx={{ 
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems="center" justifyContent="space-between">
          ...
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            {/* Add Term Button - Only show if user has create permission */}
            {canCreate && (
              <Button
                variant="contained"
            ...
            <TableBody>
              {loading ? (
                  ...
                  <TableCell colSpan={canDelete ? 6 : 5} align="center" sx={{ py: 6 }}>
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography variant="body1" sx={{ fontSize: '0.875rem', color: COLORS.text.secondary, fontWeight: 500 }}>
                        {searchTerm ? 'No terms found' : 'No terms available'}
                      </Typography>
                      <Typography variant="body2" sx={{ fontSize: '0.75rem', color: COLORS.text.tertiary, mt: 0.5 }}>
                        {searchTerm ? 'Try adjusting your search terms' : 'Add your first term to get started'}
                      </Typography>
                    </Box>
                  </TableCell>
                </TableRow>
                ...
                termsData.map((term) => {
                  ...
                  const avatarColor = getAvatarColor(term.Title);
                    ...
                    >
                      ...
                      {canDelete && (
                        <TableCell padding="checkbox" sx={{ width: 40 }}>
                          <Checkbox
                            checked={isSelected}
                            onChange={() => handleSelect(term._id)}
                            sx={{
                      ...
                      <TableCell align="center" sx={{ width: 60 }}>
                        <ActionMenu 
                          item={term}
                          onView={openViewTermModal}
                          onEdit={openEditTermModal}
                          onDelete={openDeleteTermDialog}
                          anchorEl={isActionMenuOpen ? actionMenuAnchor : null}
                          onClose={handleActionMenuClose}
                          onOpen={(e) => handleActionMenuOpen(e, term)}
