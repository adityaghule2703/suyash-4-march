...
const EditProcessDetails = ({ open, onClose, processDetail, onUpdate }) => {
  ...
  useEffect(() => {
    if (processDetail) {
      setFormData({
        PartNo: processDetail.PartNo || '',
        OperationDescription: processDetail.OperationDescription || '',
        Operation: processDetail.Operation || '',
        Machine: processDetail.Machine || '',
        Manday: processDetail.Manday || 0,
        Rate: processDetail.Rate || 0
      });
    }
  }, [processDetail]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`${BASE_URL}/api/process-details/${processDetail._id}`, formData, {
        headers: {
          'Authorization': `Bearer ${token}`,
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to update process details');
      }
    } catch (err) {
      console.error('Error updating process details:', err);
      setError(err.response?.data?.message || 'Failed to update process details. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <DialogActions sx={{
        ...
        >
          {loading ? 'Updating...' : 'Update Process Details'}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
export default EditProcessDetails;