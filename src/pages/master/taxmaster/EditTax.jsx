...
import { Edit as EditIcon, Close as CloseIcon, Error as ErrorIcon } from '@mui/icons-material';
import axios from 'axios';
import BASE_URL from '../../../config/Config';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showError(response.data.message || 'Failed to update tax');
      }
    } catch (err) {
      console.error('Error updating tax:', err);
      const errorMessage = err.response?.data?.message || 'Failed to update tax. Please try again.';
