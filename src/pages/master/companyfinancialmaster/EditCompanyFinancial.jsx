...
const EditCompanyFinancial = ({ open, onClose, financial, onUpdate, companies = [] }) => {
  ...
  useEffect(() => {
    if (financial) {
      setFormData({
        CompanyID: financial.CompanyID?._id || financial.CompanyID || '',
        CreditOnInputMaterialDays: financial.CreditOnInputMaterialDays || -30,
        WIPFGInventoryDays: financial.WIPFGInventoryDays || 30,
        CreditGivenToCustomerDays: financial.CreditGivenToCustomerDays || 45,
        CostOfCapital: financial.CostOfCapital || 0.1,
        OHPPercentage: financial.OHPPercentage || 10,
        ProfitPercentage: financial.ProfitPercentage || 15,
        ScrapRecoveryPercentage: financial.ScrapRecoveryPercentage || 85,
        EffectiveScrapRateMultiplier: financial.EffectiveScrapRateMultiplier || 0.85
      });

      // Set selected company
      if (financial.CompanyID) {
        setSelectedCompany(financial.CompanyID);
      } else {
        const company = companies.find(c => c._id === financial.CompanyID);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(`${BASE_URL}/api/company-financial/${financial._id}`, formData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to update company financial');
      }
    } catch (err) {
      console.error('Error updating company financial:', err);
      setError(err.response?.data?.message || 'Failed to update company financial. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <DialogTitle sx={{
        ...
        <div style={{
          ...
          Edit Company Financial Parameters
        </div>
      </DialogTitle>
      ...
      <DialogActions sx={{
        ...
        >
          {loading ? 'Updating...' : 'Update Financial Parameters'}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
export default EditCompanyFinancial;