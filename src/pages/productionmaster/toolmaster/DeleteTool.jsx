...
const DeleteTool = ({ open, onClose, tool, onDelete }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`${BASE_URL}/api/tool-master/${tool._id}`, {
        headers: {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to delete tool');
      }
    } catch (err) {
      console.error('Error deleting tool:', err);
      setError(err.response?.data?.message || 'Failed to delete tool. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent sx={{ p: 3 }}>
        ...
        <Box sx={{ 
          ...
          <Stack direction="row" spacing={1.5} alignItems="center">
            <BuildIcon sx={{ color: '#991B1B', fontSize: '1.25rem' }} />
            <Box>
