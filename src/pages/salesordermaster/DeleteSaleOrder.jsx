...
const DeleteSaleOrder = ({ open, onClose, so, onDelete }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`${BASE_URL}/api/sales-orders/${so._id}`, {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to delete Sales Order');
      }
    } catch (err) {
      console.error('Error deleting Sales Order:', err);
      setError(err.response?.data?.message || 'Failed to delete Sales Order. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent sx={{ p: 2.5 }}>
        <Box sx={{ textAlign: 'center', py: 2 }}>
          ...
          <Typography sx={{ fontSize: '0.8rem', color: COLORS.text.secondary }}>
            SO Number: <strong>{so?.so_number}</strong>
          </Typography>
...
export default DeleteSaleOrder;